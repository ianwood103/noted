...
func NewNoteRepository(Db *sql.DB) INoteRepository {
...
func (n *NoteRepository) Delete(ctx context.Context, noteId int) {
	tx, err := n.Db.Begin()
...
func (n *NoteRepository) FindAll(ctx context.Context) []model.Note {
	tx, err := n.Db.Begin()
	helper.PanicIfError(err)
	defer helper.CommitOrRollback(tx)
...
func (n *NoteRepository) FindById(ctx context.Context, noteId int) (model.Note, error) {
	tx, err := n.Db.Begin()
	helper.PanicIfError(err)
	defer helper.CommitOrRollback(tx)
...
func (n *NoteRepository) Save(ctx context.Context, note model.Note) {
	tx, err := n.Db.Begin()
	helper.PanicIfError(err)
	defer helper.CommitOrRollback(tx)
...
func (n *NoteRepository) Update(ctx context.Context, note model.Note) {
	tx, err := n.Db.Begin()
	helper.PanicIfError(err)
	defer helper.CommitOrRollback(tx)
